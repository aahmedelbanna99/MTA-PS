...
if is_admin_url:
    ...
    with top_cols[4]:
        ...
        st.divider()
        st.caption("🧪 اختبار Refresh Token فوري (من غير حفظ Secrets)")
        instant_refresh = st.text_input(
            "الصق Refresh Token جديد هنا فورًا لحظة ما تجيبه", key="instant_refresh_token"
        )
        instant_dhh = st.text_area(
            "الصق DHH_TOKEN (نفس token من localStorage) الحالي", key="instant_dhh", height=80
        )
        instant_cf_auth = st.text_area(
            "الصق CF_Authorization الحالي", key="instant_cf_auth", height=80
        )
        if st.button("🚀 جرب فورًا"):
            if instant_refresh.strip() and instant_dhh.strip():
                for use_cf in (False, True):
                    headers = {
                        "Accept": "application/json",
                        "Content-Type": "application/json",
                    ...
                    cookies = (
                        f"dhh_token={instant_dhh.strip()}; refresh_token={instant_refresh.strip()}"
                    )
                    if use_cf and instant_cf_auth.strip():
                        cookies += (
                            f"; CF_AppSession={TOKENS['CF_APP_SESSION']}"
                            f"; CF_Authorization={instant_cf_auth.strip()}"
                        )
                    headers["Cookie"] = cookies
                    try:
                        resp = requests.post(
                            "https://eg.me.logisticsbackoffice.com/api/iam-login/auth/refresh_token",
                            json={"refresh_token": instant_refresh.strip()},
                            headers=headers,
                            timeout=30,
                        )
                ...
                st.warning("لازم تحط Refresh Token و DHH_TOKEN الأول")
        if st.button("🔁 جرب Refresh Token يدوي (القيم المحفوظة)"):
            for use_cf in (False, True):
                headers = {
                    "Accept": "application/json",
                    "Content-Type": "application/json",
